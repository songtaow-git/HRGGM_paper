...
#pragma omp critical
    ...
    // These folds read Theta_pre[lam_id] as a read-only warm start.
    // They do not write back to Theta_pre[lam_id].
    // ------------------------------------------------------------
    if (Fold_k > 1) {
...
#pragma omp critical
        ...
        omp_unset_lock(&locks[alp_id]);
        // metrics
        double Likelihood_m = Likelihood_marginal_theta(Input_train, S_test_obs);
        double Entropy_lg = Entropy_theta_lg(Input_train);
