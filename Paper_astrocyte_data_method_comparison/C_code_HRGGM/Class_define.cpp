...
void Alpha_metric::Compute_score()
{
    RowMat Likelihood_vec = Likelihood_mat.colwise().mean();
    RowMat Entropy_vec = Entropy_mat.colwise().mean();
    int C = static_cast<int>(Likelihood_vec.cols());
    if (C > 1) {
        double* p = Likelihood_vec.data();
        double sum = 0.0;
        for (int j = 0; j < C; ++j) {
            sum += p[j];
        }
    ...
    C = static_cast<int>(Entropy_vec.cols());
    if (C > 1) {
        double* p = Entropy_vec.data();
        double sum = 0.0;
        for (int j = 0; j < C; ++j) {
            sum += p[j];
        }
    ...
    Score = 0.5 * Likelihood_vec - 0.5 * Entropy_vec;
}
