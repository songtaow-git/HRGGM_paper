...
void Newton_method_offdiag_lhg(Input_format& Input_l, const int Blockcol_inv)
{
	...
	for (int i = 0; i < Input_l.Max_iteration; ++i) {
		...
		for (size_t k = 0; k < Update_vec_ll.size(); ++k) {
			...
			Update_mu_ll_offdiag(Theta_t, C_x, C_y, k, Input_l.Lambda, Didx_ll,
		...
		while (true) {
			...
			if (chol_ll.info() == Eigen::Success) {
				...
				Norm_l1_ll_h1 = Input_l.Lambda * Offdiag_l1_norm_ll(Theta_t, Input_l.Nlocal);
				Norm_l1_lh_h1 = Input_l.Gamma * Input_l.Lambda * Block_l1_norm_lh(Theta_t,
					Input_l.Nlocal, Input_l.Nhidden);
				Norm_l2_lh_h1 = (1 - Input_l.Gamma) * Input_l.Alpha * Block_l2_norm_lh(Theta_t,
					Input_l.Nlocal, Input_l.Nhidden);
				Norm_l2_lg_h1 = Input_l.Alpha * Block_l2_norm_lg(Theta_t,Input_l.Nlocal, 
					Input_l.Nhidden, Input_l.Nglobal);
				Delta_f1 = (Likelihood_h1 - Likelihood[i]) + (Norm_l1_ll_h1 - Norm_l1_ll[i]) +
					(Norm_l1_lh_h1 - Norm_l1_lh[i]) + (Norm_l2_lh_h1 - Norm_l2_lh[i]) + 
					(Norm_l2_lg_h1 - Norm_l2_lg[i]);
				if (Delta_f1 <= 0) {
		...
		Nabla_lh = Nabla_lhg_LH(Input_l.Observed_cov, W_t, Theta_t,
			Input_l.Nlocal, Input_l.Nhidden, Input_l.Alpha, Input_l.Gamma);
		Update_vec_lh = Find_update_entries_lh(Theta_t, Nabla_lh, Input_l.Lambda,
			Input_l.Gamma, Input_l.Nlocal, Input_l.Nhidden);
		Find_update_entries_lh_lg_diag(Update_vec_lh, Input_l.Nlocal,Input_l.Nhidden, 
			Input_l.Nglobal, Update_vec_lg, Update_vec_diag, Update_vec_lh_lg_diag);
		...
		for (std::size_t k = 0; k < Update_vec_lh.size(); ++k) {
			...
			Update_mu_lh(Theta_t, C_x, C_y, k, Input_l.Lambda, Input_l.Alpha, 
				Input_l.Gamma, Didx_lh_lg_diag, W_t, W_T, U_T, Input_l.Observed_cov);
		}
		...
		for (size_t k = 0; k < Update_vec_lg.size(); ++k) {
			...
			Update_mu_lg(Theta_t, C_x, C_y, (k + Update_vec_lh.size()), Input_l.Alpha, Didx_lh_lg_diag,
				W_t, W_T, U_T, Input_l.Observed_cov);
		}
		// Update diag
		for (size_t k = 0; k < Update_vec_diag.size(); ++k) {
			int C_x = Update_vec_diag[k].first;
			Update_mu_diag(C_x, (k + Update_vec_lh.size() + Update_vec_lg.size()),
				Didx_lh_lg_diag, W_t, W_T, U_T, Input_l.Observed_cov);
		}
		// Update X_t/W_t
		h = 0;
		...
		while (true) {
			...
			if (chol_lhg.info() == Eigen::Success) {
				...
				Norm_l1_ll_h2 = Input_l.Lambda * Offdiag_l1_norm_ll(Theta_t, Input_l.Nlocal);
				Norm_l1_lh_h2 = Input_l.Gamma * Input_l.Lambda * Block_l1_norm_lh(Theta_t,
					Input_l.Nlocal, Input_l.Nhidden);
				Norm_l2_lh_h2 = (1 - Input_l.Gamma) * Input_l.Alpha * Block_l2_norm_lh(Theta_t,
					Input_l.Nlocal, Input_l.Nhidden);
				Norm_l2_lg_h2 = Input_l.Alpha * Block_l2_norm_lg(Theta_t,
					Input_l.Nlocal, Input_l.Nhidden, Input_l.Nglobal);
				...
				if (Delta_f2 <= 0) {
					Delta_f = (Likelihood_h2 - Likelihood[i]) + (Norm_l1_ll_h2 - Norm_l1_ll[i]) +
						(Norm_l1_lh_h2 - Norm_l1_lh[i]) + (Norm_l2_lh_h2 - Norm_l2_lh[i]) +
						(Norm_l2_lg_h2 - Norm_l2_lg[i]);
		...
		If_converge = KKTResidual_lhg_convergence(Theta_t, Input_l.Observed_cov,
			W_t, Input_l.Nlocal, Input_l.Nhidden, Input_l.Nglobal, Input_l.Convergence_scale,
			Input_l.Lambda, Input_l.Alpha, Input_l.Gamma);
		if (If_converge) {
