...
%%
   ...
   if (length(mu0)==1); mu0 = mu0*ones(size(blk,1),1); end
%%
   dim = zeros(1,3); numblk = zeros(1,3); 
   sdpblkidx = [];    
   for p = 1:size(blk,1)
      pblk = blk(p,:); 
      if strcmp(pblk{1},'s')
      ...
      elseif strcmp(pblk{1},'u')
         ...
         numblk(3) = numblk(3) + length(pblk{2}); 
         %%error(' unrestricted block is not allowed');
      elseif strcmp(pblk{1},'q')
         error(' SOCP block is not allowed');
      end
   end   
...
%%
   ublkinfo = zeros(size(blk,1),1); 
   for p = 1:size(blk,1)
      pblk = blk(p,:); 
      if strcmp(pblk{1},'u')
         blk{p,1} = 'l'; blk{p,2} = 2*blk{p,2}; 
         At{p} = [At{p}; -At{p}]; 
         C{p}  = [C{p}; -C{p}];
         ublkinfo(p) = 1; 
         mu0(p) = 0; 
...
%%
   if (printlevel)
      ...
      if dim(2); % fprintf('\n dim. of linear var = %2.0d',dim(2)); 
      end
      if dim(3); % fprintf('\n dim. of free var = %2.0d ',dim(3)); 
      end
      % fprintf('\n');      
   end
...
%%
   ...
   if (scale_data==2)
      par.dim = dim; 
      [X,y,Z,par,iter,sig0] = ADMMdual(blk,At,C,b,L,mu,par,X,y,Z); 
      %%[X,y,Z,par,iter,sig0] = ADMMprimal(blk,At,C,b,mu,par,X,y,Z); %% not good
      normX = ops(X,'norm'); 
      normZ = ops(Z,'norm'); 
      normZorg = normZ; 
      normXorg = normX;   
...
%%
   ...
   Rp   = b-AX; 
   normRp = norm(b-AX); 
   normRd = sqrt(abs(normX^2+normXold^2-2*blktrace(blk,X,Xold)))/sig; 
   if (use_smoothing)
...
%    if (iter > 0)
%       fprintf('\n  number of initial iterations = %2.1d',iter); 
%       fprintf('\n  sig0 = %3.2e',sig0); 
%    end
%    if (scale_data)
%       fprintf('\n  orig-normX = %2.1e, orig-normZ = %2.1e',normXorg,normZorg);  
...
%%
   for iter = 1:maxiter 
	 ...
	 if (use_smoothing)
         ...
         %%-----------------------------------------
         normRdsub = sqrt(abs(normX^2+normXold^2-2*blktrace(blk,X,Xold)))/sig;
	 const = 0.2; 
	 ...
	 par.stagnate_check_psqmr = stagnate_check_psqmr;
	    ...
	    dual_ratio = dualinf_sub/subhist.dualinf(itersub-1); 
         ...
         rhs = GradLy;
         if (iter <= 2 & itersub <= 4) %% added: 17-Nov-2010
