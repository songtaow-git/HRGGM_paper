...
struct Alpha_metric
{
	...
	RowMat Entropy_mat;             // Fold_k x N_Alpha, original normalized entropy E(alpha)
	RowMat Global_magnitude_mat;    // Fold_k x N_Alpha, M_G(alpha)
	RowMat Retention_vec;           // 1 x N_Alpha, R_G(alpha)
	RowMat Adjusted_entropy_vec;    // 1 x N_Alpha, E_adj(alpha) = E(alpha) * R_G(alpha)
	RowMat Score;                   // 1 x N_Alpha, S_new(alpha)
	Alpha_metric(const int N_f, const int N_a);
	void Compute_score(const double Tau = 0.20, const double Epsilon = 1e-12);
};
