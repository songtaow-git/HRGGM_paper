    ...
    Input_format& Input_l, const Eigen::MatrixXd& Cov_test)
{
    ...
    if (Nr <= 0 || Nr > 5)
        throw std::runtime_error("Likelihood_marginal_theta: expected 1 to 5 hidden/global features.");
    ...
    const Eigen::MatrixXd X = ldlt.solve(LR.transpose());
    Eigen::MatrixXd Theta_marg_LL = LL;
    Theta_marg_LL.noalias() -= LR * X;

    const double Logdet = Logdet_dense(Theta_marg_LL);
    const double Trace = Trace_dense_dense(Cov_test, Theta_marg_LL);
    const double Likelihood = Trace - Logdet;

    // M_G(alpha) = || Theta_LG Theta_GG^{-1} Theta_GL ||_F /
    //              (|| Theta_marg_LL ||_F + eps).
    // This is evaluated without allocating the Nl x Nl global-contribution
    // matrix: for A=Theta_LG, C=Theta_GG^{-1},
    // ||A C A^T||_F^2 = trace((C * (A^T A))^2).
    double Global_magnitude = 0.0;
    if (Ng > 0)
    {
        const Eigen::MatrixXd LG = LR.rightCols(Ng);
        const Eigen::MatrixXd GG = RR.bottomRightCorner(Ng, Ng);
        Eigen::LDLT<Eigen::MatrixXd> ldlt_GG(GG);
        if (ldlt_GG.info() != Eigen::Success)
            throw std::runtime_error("LDLT factorization of GG failed.");
        const Eigen::MatrixXd Gram = LG.transpose() * LG;
        const Eigen::MatrixXd CGram = ldlt_GG.solve(Gram);
        const double B_sq = std::max(0.0, (CGram * CGram).trace());
        Global_magnitude = std::sqrt(B_sq) / (Theta_marg_LL.norm() + eps);
    }

    return std::make_pair(Likelihood, Global_magnitude);
}

double Likelihood_marginal_theta(Input_format& Input_l, const Eigen::MatrixXd& Cov_test)
{
    return Likelihood_marginal_theta_and_global_magnitude(Input_l, Cov_test).first;
}
